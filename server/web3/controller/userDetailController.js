...
const earnUserPoints = async (req, res, next) => {
    try {
        const data = req.body
        // Get user's total earned points
        let result = await mintAndEarnPoints(data.walletAddress, data.pointsToEarn, req.body.orderId, req.body.orderName)
...
const redeemPoints = async (req, res, next) => {
    try {
        const data = req.body
        // Get user's total earned points
        let result = await redeemUserPoints(data.walletAddress, data.pointsToRedeem, req.body.orderId, req.body.orderName)
